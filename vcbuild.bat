...
if /i "%1"=="test-ci-native" set test_args=%test_args% %test_ci_args% -J --repeat 256 -p tap --logfile test.tap %CI_NATIVE_SUITES% %CI_DOC%&set build_addons=1&set build_js_native_api_tests=1&set build_node_api_tests=1&set cctest_args=%cctest_args% --gtest_output=xml:cctest.junit.xml&goto arg-ok
if /i "%1"=="test-ci-js"    set test_args=%test_args% %test_ci_args% -J --repeat 256 -p tap --logfile test.tap %CI_JS_SUITES%&set no_cctest=1&goto arg-ok
